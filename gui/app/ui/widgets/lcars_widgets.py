...
class LcarsButton(LcarsWidget):
    ...
    def __init__(self, colour, pos, text, handler=None, rectSize=None):
        if rectSize == None:
            ...
            size = (image.get_rect().width, image.get_rect().height)
            image = pygame.transform.smoothscale(image, (size[0] + 50, size[1] + 25))
        else:
            size = rectSize
            image = pygame.Surface((size[0] + 72, size[1] + 90)).convert_alpha()
            image.fill(colour)
...
class LcarsText(LcarsWidget):
    ...
    def __init__(self, colour, pos, message, size=1.0, background=None, handler=None):
        ...
        self.font = Font("assets/lcars.ttf", int(19.0 * size))
